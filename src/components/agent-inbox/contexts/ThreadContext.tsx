...
// Define the InputSchemaField type here to be used in both ThreadContext and AgentTriggerForm
...
>({ children }: { children: React.ReactNode }) {
  ...
  React.useEffect(() => {
    ...
    const inboxSearchParam = getSearchParam(INBOX_PARAM);
    if (!inboxSearchParam) {
      return;
    }
    // Don't fetch threads if we're in the "start" view
    if (inboxSearchParam === START_VIEW_PARAM_VALUE) {
      return;
    }
    try {
      fetchThreads(inboxSearchParam as ThreadStatusWithAll);
    } catch (e) {
  ...
  const fetchSchema = React.useCallback(
    async (graphId: string): Promise<InputSchemaField[]> => {
      ...
      try {
        ...
        console.log('Raw schema response:', JSON.stringify(rawSchema, null, 2));
